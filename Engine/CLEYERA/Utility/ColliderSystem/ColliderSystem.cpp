...
void CLEYERA::Manager::ColliderSystem::Update() {
  ...
  for (const auto &obj : Manager::ObjectManager::GetInstance()->GetObjects()) {

    for (const auto &it : obj.second) {
      if (!it.second) {
        continue;
      }

      auto collider = it.second->GetCollider().lock();
      if (collider) {
        mortonMap[collider->GetMortonNum()].push_back(it.second);
      }
    }
    ...
    while (true) {
      ...
      for (int i = 0; i < size; ++i) {
        ...
        for (int j = i + 1; j < size; ++j) {
          ...
          if (Util::Collider::system::Func::AABBCheck(typeA->GetAABB(),
                                                      typeB->GetAABB())) {
