...
void CLEYERA::Manager::GlobalVariables::CreateGroup(const ResourcesGroupDirectory &d, const std::string &name, std::string file) {
   datas_[name];
...
void CLEYERA::Manager::GlobalVariables::LoadFile(const std::string &DirectoryPath, const std::string &groupName) {
   using Vector2 = Math::Vector::Vec2;
   using Vector3 = Math::Vector::Vec3;
   using Vector4 = Math::Vector::Vec4;
...
void CLEYERA::Manager::GlobalVariables::LoadFiles(const std::string &DirectoryPath) {

   std::vector<fs::path> allFiles;
   std::string DirectoryfilePath = "Resources/" + DirectoryPath;

   for (const auto &entry : fs::recursive_directory_iterator(DirectoryfilePath)) {
...
void CLEYERA::Manager::GlobalVariables::ImGuiUpdate(const std::string name) {
   ...
   if (ImGui::TreeNode("json")) {
      ...
      // 各項目について
      for (std::map<std::string, Item>::iterator itItem = data.items.begin(); itItem != data.items.end(); ++itItem) {
         // 各項目を取得
         const std::string &itemName = itItem->first;
         // 項目の参照を取得
         Item &item = itItem->second;
