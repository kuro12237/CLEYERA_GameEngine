...
void ItemManager::Init() {
  // クラス名
  ManagerComponent::name_ = VAR_NAME(ItemManager);
...
void ItemManager::RegisterItemm(const std::shared_ptr<IItem> &item) { item; }
...
void ItemManager::RegisterAttackPickup(const Math::Vector::Vec3 &popPosition) {
  ...
  if (attackPickupCount_ == 0) {
    ...
    char name[32];
    std::snprintf(name, sizeof(name), "AttackPickupItem.%03zu",
                  static_cast<size_t>(attackPickupCount_));
    item->SetName(name);
  }
...
void ItemManager::RegisterHealItem(const Math::Vector::Vec3 &popPosition) {
  ...
  if (healCount_ == 0) {
    ...
    char name[32];
    std::snprintf(name, sizeof(name), "HealItem.%03zu",
                  static_cast<size_t>(healCount_));
    item->SetName(name);
  }
