...
CreateOrientations(const Math::Vector::Vec3 &rotate);

void MakeLinesFromOBB(std::vector<Math::Vector::Vec3> &outLines,
                      const OBB &obb);
...
// 頂点を軸に射影
std::pair<float, float> OBBProjection(const OBB &obb,
...
bool CalculateMTV(const Util::Collider::system::OBB &obbA,
...
GetVertices(const Util::Collider::system::OBB &obb);

Math::Vector::Vec3 PushOutOBB(Util::Collider::system::OBB &obbA,
