...
bool CLEYERA::Util::Collider::Octree::IsInside(const Math::Vector::Vec3 &p) const {
   const Vector3 min = GetMin();
   const Vector3 max = GetMax();
   return (p.x >= min.x && p.x <= max.x) && (p.y >= min.y && p.y <= max.y) && (p.z >= min.z && p.z <= max.z);
}

int CLEYERA::Util::Collider::Octree::GetMortonNumber(Math::Vector::Vec3 position) {
...
int CLEYERA::Util::Collider::Octree::GetMortonNumber(Math::Vector::Vec3 min, Math::Vector::Vec3 max) {
...
DWORD CLEYERA::Util::Collider::Octree::GetMortonOrder(const Math::Vector::Vec3 &point) {
